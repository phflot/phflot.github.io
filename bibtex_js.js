...
function BibtexDisplay() {
    ...
    // 1) If you have any restrictions by bibtexkeys, do that first:
    if (output[0].attributes.length > 1) {
        for (var i = 0, attrs = output[0].attributes, n = attrs.length; i < n; ++i) {
            var attrName = attrs[i].nodeName.toUpperCase();
            var attrValue = attrs[i].nodeValue;
            if (attrName in entry) {
                if (!entry[attrName].match(attrValue)) return null;
            }
        }
    }

    // 2) Clone your template element (remove .bibtex_template class)
    var tpl = $(".bibtex_template").clone().removeClass("bibtex_template");

    // 3) Fill in fields from the entry
    var keys = [];
    for (var k in entry) {
    ...
    for (var idx in keys) {
        ...
        if (key === "BIBTEXRAW") {
            tpl.find("." + key.toLowerCase()).html(val);
            continue;
        }

        if (key === "AUTHOR") {
            var authorFormat = tpl.find("span:not(a)." + key.toLowerCase());
            if (authorFormat.length) {
                val = this.displayAuthor(val, authorFormat);
            }
        } else if (key === "PAGES") {
            val = val.replace("--", "-");
        } else if (key === "URL") {
            ...
            // Default fallback: fix latex, etc.
            val = this.fixValue(val);
        }

        // Insert the text or link href
        tpl.find("span:not(a)." + key.toLowerCase()).html(val);
    ...
    // 4) Bolding “Flotho” – your original snippet unchanged
    tpl.find(".author").each(function() {
        const before = $(this).html();
        console.log("Before bolding:", before);

        const $target = $(this).find("span[class='P. Flotho']");
        console.log("Found target spans:", $target.length); // debug
        ...
        const after = $(this).html();
        console.log("After bolding:", after);
    });

    // 5) Now handle punctuation & formatting for “title”, “journal”, “volume”, etc.
    //    We only add punctuation if the field is present, otherwise we remove that <span> block.

    // Title => add comma + quotes around it. Example:  , "Title"
    if (entry["TITLE"]) {
        tpl.find("span.if.title").prepend(', "').append('"');
    } else {
        tpl.find("span.if.title").remove();
    }

// 1) If this entry is an ARTICLE
if (entry["BIBTEXTYPEKEY"] === "ARTICLE") {
  // Show .if.journal, prepend ", in " and wrap content in <em>
  if (entry["JOURNAL"]) {
    tpl.find("span.if.journal").each(function() {
      const journalContent = entry["JOURNAL"];
      $(this).html(', in <em>' + journalContent + '</em>');
    });
  } else {
    tpl.find("span.if.journal").remove();
  }
  tpl.find("span.if.booktitle").remove(); // Remove booktitle for articles
}

// 2) If this entry is INPROCEEDINGS
else if (entry["BIBTEXTYPEKEY"] === "INPROCEEDINGS") {
  // Show .if.booktitle, prepend ", in " and wrap content in <em>
  if (entry["BOOKTITLE"]) {
    tpl.find("span.if.booktitle").each(function() {
      const bookTitleContent = entry["BOOKTITLE"];
      $(this).html(', in <em>' + bookTitleContent + '</em>');
    });
  } else {
    tpl.find("span.if.booktitle").remove();
  }
  tpl.find("span.if.journal").remove(); // Remove journal for inproceedings
}


// 3) If neither, or something else, remove both so we don't get leftover punctuation
else {
  tpl.find("span.if.journal").remove();
  tpl.find("span.if.booktitle").remove();
}
    ...
    if (entry["VOLUME"]) {
        tpl.find("span.if.volume").prepend(', vol. ');
    } else {
        tpl.find("span.if.volume").remove();
    }

    // Pages => preceded by “, pp. ”
    if (entry["PAGES"]) {
        tpl.find("span.if.pages").prepend(', pp. ');
    } else {
        tpl.find("span.if.pages").remove();
    }

    // Year => preceded by “, ” and followed by period
    if (entry["YEAR"]) {
        tpl.find("span.if.year").prepend(', ').append('.');
    } else {
        tpl.find("span.if.year").remove();
    }

    // DOI => if present, we can leave it as a link (the code above sets the link).
    //        If you want a space or punctuation before it, do so here:
if (entry["DOI"]) {
    // Find the span for DOI and insert a clickable link
    tpl.find("span.if.doi").each(function () {
        const doiValue = entry["DOI"];
        $(this).html('<a href="https://doi.org/' + doiValue + '" target="_blank">' + doiValue + '</a>');
    });
} else {
    tpl.find("span.if.doi").remove();
}


    // 6) Mark the final node as a bibtex entry and return
